...
muh_stocks = [
    # some aus stock? idk. TODO: actually read the fucking article
    'EWA',
    'EWC',
    ...
    # YOINK from https://towardsdatascience.com/find-highly-correlated-stocks-with-python-77eba4fd061b
    'GS',
    'JPM',
    'AXP',
    'HON',
...
# true if we are backtesting, else we use defaults
useParams = True
...
class EMAMomentumUniverse(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        if useParams:  # if we r backtesting
            # it was 220 originally
            lookback = int(self.GetParameter("lookback"))
            stock1idx = int(self.GetParameter("stock1idx"))
            stock2idx = int(self.GetParameter("stock2idx"))
        else:
            # lookback = 220 #(it was...)
            # but backtest says this is better:
            lookback = 250
            stock1idx = 0
            stock2idx = 1

        stock1 = muh_stocks[stock1idx]
        stock2 = muh_stocks[stock2idx]

        # Add the assets to be fed into the algorithm and save the symbol objects (to be referred later)
        self.asset1 = self.AddEquity(stock1, Resolution.Daily).Symbol
        self.asset2 = self.AddEquity(stock2, Resolution.Daily).Symbol

        # We then create a bollinger band with 120 steps for lookback period
        self.bb = BollingerBands(lookback, 0.5, MovingAverageType.Simple)
        ...
        self.long_targets = [PortfolioTarget(
            self.asset2, 0.9), PortfolioTarget(self.asset1, -0.9)]
        self.short_targets = [PortfolioTarget(
            self.asset2, -0.9), PortfolioTarget(self.asset1, 0.9)]

        self.is_invested = None         # set the starting flag as not invested

        # set the look back period to calculate the linear regression and find the hedge ratio
    ...
    def OnData(self, data):
        ...
        portfolio = data[self.asset2].Close - \
            data[self.asset1].Close * reg.slope - reg.intercept
